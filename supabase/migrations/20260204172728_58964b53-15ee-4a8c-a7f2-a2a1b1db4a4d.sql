-- Fix create_service_request_bundle: bypass RLS, return real errors, prevent orphan submissions
...
BEGIN
  ...
  -- Guardrail: prevent orphan submissions (no valid items processed)
  IF array_length(v_request_ids, 1) IS NULL OR array_length(v_request_ids, 1) = 0 THEN
    ...
    RETURN jsonb_build_object(
      ...
      'message', 'No valid items found for this request.'
    );
  END IF;
...
EXCEPTION WHEN OTHERS THEN
  -- Return real database error for debugging
  RETURN jsonb_build_object(
    'success', false,
    'error', SQLSTATE,
    'message', SQLERRM,
