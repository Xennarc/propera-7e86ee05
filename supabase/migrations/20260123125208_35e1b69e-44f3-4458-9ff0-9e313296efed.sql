-- Fix guest_get_room_bookings RPC: remove non-existent columns (cuisine_type, image_url)
-- and fix guest_cancel_cutoff_hours → guest_cancel_cutoff_minutes
...
BEGIN
  ...
  -- Restaurant reservations - FIXED: removed cuisine_type/image_url, fixed cutoff field name
  SELECT COALESCE(jsonb_agg(
    jsonb_build_object(
      'id', rr.id,
