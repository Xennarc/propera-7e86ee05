-- Fix guest_get_room_bookings: correct restaurant_reservations column names
-- Issues: rr.slot_id should be rr.restaurant_slot_id, rr.party_size should be rr.num_adults/num_children
...
BEGIN
  ...
  -- Get restaurant reservations with CORRECTED column names
  -- FIXED: rr.restaurant_slot_id (not slot_id), rr.num_adults/num_children (not party_size)
  SELECT COALESCE(jsonb_agg(
    jsonb_build_object(
      'id', rr.id,
  ...
  JOIN restaurant_time_slots ts ON ts.id = rr.restaurant_slot_id  -- FIXED: was rr.slot_id
...
GRANT EXECUTE ON FUNCTION public.guest_get_room_bookings(uuid) TO anon, authenticated;