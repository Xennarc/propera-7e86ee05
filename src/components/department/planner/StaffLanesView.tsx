 ...
 * and session assignment blocks overlaid. View-only for non-managers.
 */
...
export function StaffLanesView({ resortId, deptKey, date, onSessionClick }: Props) {
  ...
  return (
    <div className="space-y-1">
      ...
      {/* Staff lanes */}
      {members.map(member => {
        const userShifts = shiftsByUser[member.id] ?? [];
        const userAssignments = assignmentsByUser[member.id] ?? [];

        return (
          <div key={member.id} className="flex items-stretch group">
            ...
            <div className="flex-1 relative h-12 bg-muted/20 rounded border border-border/30">
              {/* Shift backgrounds */}
              {userShifts.map(shift => (
                <div
                  key={shift.id}
                  className="absolute top-0 bottom-0 bg-primary/5 border-y border-primary/10"
                  style={{
                    left: `${timeToPercent(shift.start_time)}%`,
                    width: `${timeToPercent(shift.end_time) - timeToPercent(shift.start_time)}%`,
                  }}
                />
              ))}
                ...
                return (
                  <Tooltip key={a.id}>
                    <TooltipTrigger asChild>
                      <button
                        className={cn(
                          'absolute top-1 bottom-1 rounded px-1 text-[10px] font-medium truncate cursor-pointer transition-colors',
                          isOutsideShift
