  ...
  return (
    <div className="bg-card rounded-xl border border-border/50 p-4 shadow-md">
      ...
      <div className="h-8 bg-gradient-to-r from-primary/20 via-primary/40 to-primary/20 rounded-lg flex items-center px-3">
        <div className="h-2 bg-primary rounded-full w-3/4 transition-all duration-700" />
      </div>
...
const SessionsMockup = memo(function SessionsMockup({ 
  sessions 
}: { 
  sessions: { time: string; name: string; pax: string; status: string }[];
}) {
  return (
    <div className="bg-card rounded-xl border border-border/50 p-4 shadow-md">
...
const PortfolioMockup = memo(function PortfolioMockup({ 
  resorts 
}: { 
  resorts: { name: string; occ: string }[];
}) {
  return (
    <div className="bg-card rounded-xl border border-border/50 p-4 shadow-md">
...
// Memoized storyboard panel
const StoryboardPanel = memo(function StoryboardPanel({ 
  panel, 
  index,
  shouldAnimate 
}: { 
  panel: typeof storyboardPanels[0]; 
  index: number;
      ...
      <div className="bg-background rounded-3xl border border-border/50 shadow-lg overflow-hidden">
        ...
        <div className="grid md:grid-cols-2">
          ...
          <div className="p-6 md:p-8 relative">
            <div className="absolute top-4 left-4 flex items-center gap-2">
              <div className={`w-3 h-3 bg-primary rounded-full ${shouldAnimate ? 'animate-pulse' : ''}`} />
            ...
            <div className="mt-8">
              {panel.after.mockup === 'dashboard' && panel.after.metrics && (
                <DashboardMockup metrics={panel.after.metrics} />
              )}
              {panel.after.mockup === 'sessions' && panel.after.sessions && (
                <SessionsMockup sessions={panel.after.sessions} />
              )}
              {panel.after.mockup === 'portfolio' && panel.after.resorts && (
                <PortfolioMockup resorts={panel.after.resorts} />
              )}
            </div>
...
const BenefitCard = memo(function BenefitCard({ 
  ...
  return (
    <motion.div 
      ...
      viewport={{ once: true }}
      className="bg-background rounded-2xl border border-border/50 p-6 hover:border-primary/30 hover:-translate-y-1 transition-all duration-300 group will-change-transform"
    >
