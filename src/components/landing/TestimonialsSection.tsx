...
const testimonials = [
  ...
  },
  {
    ...
    bgSlice: 'portfolio' as const,
  },
];

// Memoized background slice - CSS-only shapes for performance
const TestimonialBgSlice = memo(function TestimonialBgSlice({ type }: { type: string }) {
          ...
          <div className="relative bg-background rounded-3xl border border-border/50 shadow-elevated overflow-hidden">
            ...
            <div className="absolute inset-0 opacity-5">
              <AnimatePresence mode="wait">
                ...
                >
                  <TestimonialBgSlice type={currentTestimonial.bgSlice} />
                </motion.div>
              </AnimatePresence>
            </div>
            ...
            <div className="relative p-8 md:p-12">
              ...
              {/* Content with minimal animation */}
              <AnimatePresence mode="wait">
                <motion.div
                  key={current}
                ...
                >
                  ...
                  <div className="flex items-center justify-center gap-4">
                    <div className="w-14 h-14 rounded-full bg-gradient-to-br from-primary to-teal-400 flex items-center justify-center text-primary-foreground font-bold text-lg shadow-lg">
                      {currentTestimonial.avatar}
                    </div>
                    <div className="text-left">
                      <p className="font-semibold text-foreground">{currentTestimonial.author}</p>
                      <p className="text-sm text-muted-foreground">{currentTestimonial.role}</p>
                      <p className="text-sm text-primary">{currentTestimonial.property}</p>
                    </div>
                  </div>
                  
                  {/* Property type tag */}
                  <div className="mt-4 inline-flex items-center gap-2 bg-muted/50 px-3 py-1.5 rounded-full text-xs text-muted-foreground">
