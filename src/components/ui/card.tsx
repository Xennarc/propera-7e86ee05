...
const cardVariants = cva(
  "rounded-2xl border bg-card text-card-foreground transition-all duration-200",
  {
    variants: {
      variant: {
        default: "border-border/40 shadow-soft",
        elevated: "border-border/40 shadow-medium hover:shadow-elevated",
        glass: "border-border/30 bg-card/80 backdrop-blur-sm",
        interactive: "border-border/40 shadow-soft hover:shadow-medium hover:border-border/60 cursor-pointer",
      },
    },
    defaultVariants: {
      variant: "default",
...
const CardTitle = React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLHeadingElement>>(
  ({ className, ...props }, ref) => (
    <h3 ref={ref} className={cn("text-base sm:text-lg font-semibold leading-none tracking-tight text-foreground", className)} {...props} />
  ),
);
